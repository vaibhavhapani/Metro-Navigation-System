...
public class Main {
	...
	public boolean containsEdge(String vertexName1, String vertexName2) {
		Vertex vtx1 = vertices.get(vertexName1);
		Vertex vtx2 = vertices.get(vertexName2);

		if (vtx1 == null || vtx2 == null || !vtx1.neighbours.containsKey(vertexName2)) {
			return false;
		}
	...
	public void addEdge(String vertexName1, String vertexName2, int value) {
		Vertex vtx1 = vertices.get(vertexName1);
		Vertex vtx2 = vertices.get(vertexName2);
		...
		vtx1.neighbours.put(vertexName2, value);
		vtx2.neighbours.put(vertexName1, value);
	}
	...
	public void removeEdge(String vertexName1, String vertexName2) {
		...
		vtx1.neighbours.remove(vertexName2);
		vtx2.neighbours.remove(vertexName1);
	}
	...
	public void display_Map() {
		...
		for (String key : keys) {
			String str = key + " =>\n";
			Vertex vtx = vertices.get(key);
			ArrayList<String> vtxNeighbour = new ArrayList<>(vtx.neighbours.keySet());

			for (String nbr : vtxNeighbour) {
				str = str + "\t" + nbr + "\t";
				if (nbr.length() < 16)
					str = str + "\t";
				if (nbr.length() < 8)
	...
	public boolean hasPath(String vertexName1, String vertexName2, HashMap<String, Boolean> processed) {
		// DIR EDGE
		...
		// MARK AS DONE
		processed.put(vertexName1, true);

		Vertex vtx = vertices.get(vertexName1);
		...
		for (String nbr : nbrs) {
			...
			if (!processed.containsKey(nbr))
				if (hasPath(nbr, vertexName2, processed))
					return true;
		}
	...
	private class DijkstraPair implements Comparable<DijkstraPair> {
		...
		/*
		 * Removing the overriden method gives us this errror: The type
		 * Main.DijkstraPair must implement the inherited abstract method
		 * Comparable<Main.DijkstraPair>.compareTo(Main.DijkstraPair)
		 * 
		...
		while (!heap.isEmpty()) {
			...
			for (String nbr : v.neighbours.keySet()) {
				if (map.containsKey(nbr)) {
					...
					int nc;
					if (nan)
						nc = rp.cost + 120 + 40 * k.neighbours.get(nbr);
					else
						nc = rp.cost + k.neighbours.get(nbr);
	...
	public String Get_Minimum_Distance(String src, String dst) {
		...
		while (!stack.isEmpty()) {
			...
			Vertex rpvtx = vertices.get(rp.vertexName);
			ArrayList<String> nbrs = new ArrayList<>(rpvtx.neighbours.keySet());
			...
			for (String nbr : nbrs) {
				...
				if (!processed.containsKey(nbr)) {
					...
					np.vertexName = nbr;
					np.psf = rp.psf + nbr + "  ";
					np.min_dis = rp.min_dis + rpvtx.neighbours.get(nbr);
		...
		while (!stack.isEmpty()) {
			...
			for (String nbr : nbrs) {
				...
				if (!processed.containsKey(nbr)) {
					...
					np.min_time = rp.min_time + 120 + 40 * rpvtx.neighbours.get(nbr);
					stack.addFirst(np);
				}
			}
		}
